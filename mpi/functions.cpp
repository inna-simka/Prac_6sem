    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            cout << fixed << setiosflags(ios::left) << setw(6)
                      << setfill(' ') << setprecision(2) << mtx[i*cols+j];
        }
        cout << endl;
    }
    cout << endl;
...
void Matrix::LLtDecomposition()
{
    if (rows != cols) {
        cout << "Cannot factorize non-square matrix." << endl;
        exit(10);
    }
    double *LLt = new double[rows * cols];
    for (int i = 0; i < rows; ++i) {
        for (int j = i; j < cols; ++j) {
            double sum = 0;
            for (int k = 0; k < i; ++k) {
                sum += LLt[i*cols + k] * LLt[k*cols + j];
            }
            LLt[i*cols+j] = (mtx[i*cols + j] - sum);
        }

        for (int j = i + 1; j < cols; ++j) {
            double sum = 0;
            for (int k = 0; k < i; ++k) {
                sum += LLt[j*cols + k] * LLt[k*cols + i];
            }
            LLt[j*cols + i] = (mtx[j*cols + i] - sum) / LLt[i*cols + i];
        }
    }
    mtx = LLt;
}
...
double* &Matrix::get_block(int i, int j)
{
    if (block_mtx) {
        ...
        cout << "Failed to get matrix block " << i << " " << j << endl;
        exit(0);
    }
}
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            if (i > j) {
                ...
                Lt.ptr()[i*cols + j] = A.ptr()[i*cols + j];
            } else if (i == j) {
                L.ptr()[i*cols + j] = 1;
                Lt.ptr()[i*cols + j] = A.ptr()[i*cols + j];
            }
...
{
    if (M1.cols != M2.rows) {
        cout << "Cannot multiply matrices of these dimensions." << endl;
        exit(1);
    }
...
Matrix operator-(Matrix M1, Matrix M2)
{
    if (M1.cols != M2.cols or M1.rows != M2.rows) {
        cout << "Cannot subtract matrices of these dimensions." << endl;
        exit(1);
    }
...
{
    Matrix L = get<0>(extractLU(LU));
    Matrix U = get<1>(extractLU(LU));
    Matrix A_PREC = L * U - A;
 //   A_PREC.print();
    double norm = 0;
